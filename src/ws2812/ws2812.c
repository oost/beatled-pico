...
uint64_t calculate_beat_fraction(uint64_t current_time) {
  uint64_t duration_from_time_ref =
      (current_time - registry.last_beat_time_ref);
  return (duration_from_time_ref % registry.tempo_period_us) * UINT32_MAX /
         registry.tempo_period_us;
}

void advance_time_ref(uint64_t current_time) {
...
void led_update() {
  ...
  advance_time_ref(current_time);

  uint64_t beat_frac = calculate_beat_fraction(current_time);
  // printf(C"%llu\n", beat_frac);
