...
void pattern_fade_grey(uint32_t *stream, size_t len, uint8_t t,
  ...
  // Right-shift by 4 divides by 16, giving 0-15 range (conservative brightness)
  uint8_t value = beat_intensity_quadratic(t) >> 4;
...
void pattern_fade_color(uint32_t *stream, size_t len, uint8_t t,
  ...
  // Right-shift by 4 divides by 16, giving 0-15 range for gentle fade
  uint8_t value = beat_intensity_quadratic(t) >> 4;
