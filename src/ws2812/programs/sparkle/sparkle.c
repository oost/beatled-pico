...
void pattern_sparkle(uint32_t *stream, size_t len, uint8_t t,
  ...
  // Creates "burst" effect: many sparkles at beat start, then fades to occasional sparkles
  ...
  uint8_t brightness = beat_intensity_quadratic(t) >> 3;  // Right-shift by 3 gives 0-31 range
